function analyzeAvgWindow(params, pilot_pos, EbN0_domain_dB_plot, num_iterations_for_std_dev, ppm, K_values, N_fixed)
    ...
    title_str_cfo = sprintf('Impact of the averaging window (N=%d, %d-%s)', N_fixed, ModOrder, upper(ModType));
    title_str_toa = sprintf('Impact of the averaging window (N=%d, %d-%s)', N_fixed, ModOrder, upper(ModType));
    figure;
    ...
    grid on;
    xlabel('Ratio $E_b/N_0$ [dB]', 'Interpreter', 'latex', 'FontSize', 30);
    ylabel('Std. Dev. of ToA Error (in number of samples)', 'Interpreter', 'latex', 'FontSize', 30);
