function h_rrc = rrcFilter(Beta, SymRate, OSF, NumTaps)
           ...
           h_rrc = fftshift(ifft(ifftshift(H_RRC)));
           ...
           h_rrc = h_rrc / sqrt(peak_val_rc);           % Corresponding h_rrc to the Normalized h_rc      
           h_rc = conv(h_rrc, h_rrc);                   % Normalized h_rc
    
    
           % Plots
           figure;
           set(gca, 'TickLabelInterpreter', 'latex', 'FontSize', 24);
           plot(freq / 1e6, abs(H_RC), 'Color', [0 0.4470 0.7410], 'LineWidth', 2, 'Marker', 'o', 'MarkerFaceColor', [0 0.4470 0.7410], 'DisplayName', '$|H_{RC}(f)|$');
           hold on;
           ...
           xlabel('Frequency (MHz)', 'Interpreter', 'latex', 'FontSize', 24);
           ...
           h_rc_Tsymb = h_rc(sampling_indices);
           k_Tsymb = (k_values * Tsymb);
           stem(k_Tsymb * 1e6, h_rc_Tsymb, 'Color', [0 0.4470 0.7410], 'LineWidth', 4, 'MarkerFaceColor', [0 0.4470 0.7410], 'Marker', 'o', 'DisplayName', '$h_{RC}(kT_{symb})$');
           hold on;
           plot(t_rc * 1e6, h_rc, 'DisplayName', '$h_{RC}(t)$', 'Color', [0.8500 0.3250 0.0980], 'LineWidth', 2);
           hold on;
           plot(t_rrc * 1e6, h_rrc, 'Color', [0.4660 0.6740 0.1880], 'LineWidth', 2, 'LineStyle','--', 'DisplayName', '$h_{RRC}(t)$');
           hold off;
           grid on;
           set(gca, 'GridLineWidth', 2, 'GridColor', [0.3 0.3 0.3], 'GridAlpha', 0.5);

           xlabel('Time ($\mu s$)', 'Interpreter', 'latex', 'FontSize', 24);
