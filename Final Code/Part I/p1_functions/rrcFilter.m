function g = rrcFilter(Beta, SymRate, OSF, NumTaps)
           Fs = OSF * SymRate;
           Tsymb = 1 / SymRate;
           ...
           H(pass_band_idx) = Tsymb;
       
           % Region 2: Roll-off band (f1 < |f| <= f2)
           roll_off_idx = (abs(freq) > f1_hz) & (abs(freq) <= f2_hz);
           H(roll_off_idx) = (Tsymb / 2) * (1 + cos((pi * Tsymb / Beta) * (abs(freq((roll_off_idx))) - f1_hz)));
           ...
           h = conv(g, g);
           center_idx_rc = floor(length(h)/2) + 1;   % Index of the peak for zero delay
           peak_val_rc = h(center_idx_rc);
           g = g / sqrt(peak_val_rc);           % Corresponding g to the Normalized h      
           h = conv(g, g);                   % Normalized h
