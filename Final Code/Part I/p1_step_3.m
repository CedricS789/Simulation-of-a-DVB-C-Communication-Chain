...
fprintf('\n\n=============================================');
fprintf('\n BER (Multiple Modulations) Simulation Setup   ');
fprintf('\n=============================================');
fprintf('\nBase Modulation Family: %s', upper(ModType));
fprintf('\nNbps Values: %s', mat2str(nbps_values));
fprintf('\nEb/N0: %.1f-%.1f dB (step = %.1f dB)', params.simulation.EbN0_domain_dB(1), params.simulation.EbN0_domain_dB(end), params.simulation.EbN0_domain_dB(2)-params.simulation.EbN0_domain_dB(1));
...
for idx_nbps = 1:length(nbps_values)
    ...
    for idx_EbN0 = 1:num_EbN0_points
        ...
        for iter = 1:iterations 
            ...
            symb_rx = downSampler(signal_rx, OSF); 
            bit_rx = demapping_v2(symb_rx, current_nbps, ModType); 
