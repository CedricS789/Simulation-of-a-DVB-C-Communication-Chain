...
freq_shift = 10e6; % Desired frequency shift in Hz
...
%% ------ Shift the signal in frequency ------

t_shift = (0:length(tx_filtered)-1)/Fs;
freq_shift_signal = tx_filtered .* exp(1j * 2 * pi * freq_shift * t_shift.');

% Plot spectrum of the shifted signal
TX_freq_shifted = fftshift(fft(freq_shift_signal, N_fft))/length(freq_shift_signal);

figure;
plot(f_axis/1e6, 20*log10(abs(TX_freq_shifted)));
title(['Spectrum of Filtered Signal (Shifted by ' num2str(freq_shift/1e6) ' MHz)']);
xlabel('Frequency (MHz)');
ylabel('Power Spectral Density (dB)');
grid on;
xlim([-Fs/2e6 Fs/2e6]);

% Plot time domain shifted signal
figure;
subplot(2,1,1);
plot(t_sig*1e6, real(freq_shift_signal));
title('Real Part of Shifted Filtered Signal');
xlabel('Time (μs)');
ylabel('Amplitude');
grid on;

subplot(2,1,2);
plot(t_sig*1e6, imag(freq_shift_signal));
title('Imaginary Part of Shifted Filtered Signal');
xlabel('Time (μs)');
ylabel('Amplitude');
grid on;